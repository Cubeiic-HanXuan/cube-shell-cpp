...
# cube-shell: MSVC ARM64 的 Release 优化器会错误编译 OpenSSL（OpenSSL 上游
# issue #26239 与 #27030，后者在 MSVC 14.51 上仍未修复）。症状是 TLS 握手期
# 寄存器/栈被破坏，随后在 libssl 内（如 tls_parse_all_extensions 解引用
# SSL_CONNECTION）以 0xC0000005 崩溃，FreeRDP 一连 RDP 进程即消失。
# 社区确认的唯一可靠规避是关掉 C 编译优化（/Od）。OpenSSL 的加解密热路径
# 是手写 ARMv8 汇编，不受 C 编译器优化级别影响，/Od 只拖慢冷路径，对
# RDP 会话性能影响可忽略。x64 / clang 不受影响，不动。
if(VCPKG_TARGET_ARCHITECTURE STREQUAL "arm64"
   AND NOT VCPKG_DETECTED_CMAKE_C_COMPILER_ID MATCHES "Clang")
    string(REPLACE "-O2 -Oi" "/Od" VCPKG_COMBINED_C_FLAGS_RELEASE "${VCPKG_COMBINED_C_FLAGS_RELEASE}")
    string(REPLACE "-O2" "/Od" VCPKG_COMBINED_C_FLAGS_RELEASE "${VCPKG_COMBINED_C_FLAGS_RELEASE}")
    message(STATUS "cube-shell overlay: OpenSSL ARM64 release flags de-optimized to /Od: ${VCPKG_COMBINED_C_FLAGS_RELEASE}")
endif()
